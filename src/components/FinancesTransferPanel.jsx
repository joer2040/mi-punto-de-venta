...
const validateTransfer = ({ fromCode, toCode, amount }) => {
  const errors = {}
  if (!fromCode) errors.fromCode = 'Selecciona el fondo origen.'
  if (!toCode) errors.toCode = 'Selecciona el fondo destino.'
  if (fromCode && toCode && fromCode === toCode) {
    errors.toCode = 'El fondo destino debe ser diferente al origen.'
  }
...
const FinancesTransferPanel = ({ onClose, onNavigate }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    const errors = validateTransfer(form)
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setResult({
        ...
        message: `${formatCurrency(parseFloat(form.amount))} de ${FUND_NAMES[form.fromCode]} → ${FUND_NAMES[form.toCode]}.`,
      })
      ...
      setResult({
        ...
        message: err.message || 'No se pudo registrar el traspaso.',
      })
      setConfirmPending(false)
      idempotencyKeyRef.current = generateIdempotencyKey()
    } finally {
  ...
  const confirmLines = [
    `Origen: ${FUND_NAMES[form.fromCode]}`,
    `Destino: ${FUND_NAMES[form.toCode]}`,
  ...
  return (
    <div style={panelStyle}>
      <div style={panelHeaderStyle}>
        <span style={panelTitleStyle}>Traspaso entre fondos</span>
      ...
      {!confirmPending && !succeeded && (
        <form onSubmit={handleSubmit} noValidate>
          <div style={fieldGroupStyle}>
            <label htmlFor="transfer-from" style={labelStyle}>
              Fondo origen <span style={requiredMarkStyle}>*</span>
            </label>
            <select
              id="transfer-from"
              value={form.fromCode}
              onChange={handleField('fromCode')}
              style={{ ...selectStyle, ...(fieldErrors.fromCode ? errorBorderStyle : null) }}
            >
          ...
          <div style={fieldGroupStyle}>
            <label htmlFor="transfer-to" style={labelStyle}>
              Fondo destino <span style={requiredMarkStyle}>*</span>
            </label>
            <select
              id="transfer-to"
              value={form.toCode}
              onChange={handleField('toCode')}
              style={{ ...selectStyle, ...(fieldErrors.toCode ? errorBorderStyle : null) }}
            >
              {FUND_OPTIONS.map((opt) => (
                <option key={opt.code} value={opt.code}>
                  {opt.code} — {opt.name}
          ...
          <div style={fieldGroupStyle}>
            <label htmlFor="transfer-amount" style={labelStyle}>
          ...
          <div style={fieldGroupStyle}>
            <label htmlFor="transfer-description" style={labelStyle}>
              Descripción{' '}
              <span style={optionalMarkStyle}>(opcional)</span>
            </label>
            <input
...
export default FinancesTransferPanel
