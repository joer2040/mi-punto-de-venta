-- Fase 4: Ventas con pagos múltiples y asientos del ledger.
-- Tablas financial_operations + financial_payments.
-- Extiende sales con financial_operation_id y journal_entry_id.
-- Reemplaza finalize_pos_sale para soportar multi-pago + ledger atómico.

begin;

-- ============================================================
-- 1. Operaciones financieras (cabecera de negocio genérica)
-- ============================================================
...
create table if not exists public.financial_operations (
  ...
  performed_by    uuid         not null references public.app_profiles(id),
  idempotency_key text,
  created_at      timestamptz  not null default now(),
  constraint financial_operations_type_check check (
    operation_type in (
      'sale', 'purchase', 'expense', 'transfer',
      'owner_contribution', 'owner_withdrawal', 'cash_discrepancy', 'reversal'
    )
  ),
  constraint financial_operations_status_check check (
    status in ('confirmed', 'reversed')
  ),
  constraint financial_operations_amount_check check (total_amount > 0)
);

create index if not exists financial_operations_source_idx
  on public.financial_operations (source_type, source_id);

create index if not exists financial_operations_session_idx
  on public.financial_operations (cash_session_id);

create index if not exists financial_operations_entry_idx
  on public.financial_operations (journal_entry_id);

comment on table public.financial_operations is
  'Documento de negocio genérico: venta, compra, gasto, traspaso, aportación, retiro. Vincula la operación con su asiento y sus pagos.';

-- ============================================================
-- 2. Desglose de cobros/pagos por operación
-- ============================================================

create table if not exists public.financial_payments (
  id                      uuid         primary key default gen_random_uuid(),
  financial_operation_id  uuid         not null references public.financial_operations(id),
  ...
  constraint financial_payments_method_check check (
    payment_method in ('Efectivo', 'Tarjeta', 'Transferencia')
  )
);

create index if not exists financial_payments_operation_idx
  on public.financial_payments (financial_operation_id);

create index if not exists financial_payments_account_idx
  on public.financial_payments (financial_account_id);

comment on table public.financial_payments is
  'Desglose de cobros y pagos por operación financiera. Uno por método de pago.';

-- ============================================================
-- 3. Extender sales con vínculos al ledger
-- ============================================================
...
begin
  alter table public.sales
    add constraint sales_financial_operation_fkey
...
begin
  alter table public.sales
    add constraint sales_journal_entry_fkey
...
-- ============================================================
-- 5. Reemplazar finalize_pos_sale
--    Nueva firma: p_payments jsonb (en lugar de p_payment_method text)
--    Agrega: p_idempotency_key text
--    Crea asientos del ledger si está activo.
-- ============================================================

-- Eliminar firma antigua
