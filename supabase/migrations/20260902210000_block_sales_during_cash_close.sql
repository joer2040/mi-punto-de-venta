-- Sprint 2B M2: Bloquear ventas cuando caja está en proceso de cierre.
-- first_counted_cash IS NOT NULL → sesión en cierre → POS bloqueado.
--
-- Cambios:
-- 1. require_open_cash_session_for_pos_operation: agrega check first_counted_cash IS NULL
-- 2. finalize_pos_sale: agrega guard antes del bloque de sesión de caja
--
-- Mensajes de error diferenciados:
--   sin sesión abierta  → 'No hay una caja abierta...'
--   sesión en cierre    → 'La caja está en proceso de cierre...'

begin;

-- ── 1. Trigger: require_open_cash_session_for_pos_operation ────────────────
...
-- ── 2. finalize_pos_sale: agrega guard pre-caja ────────────────────────────
-- Reproducción completa de 20260815100000_fix_finalize_pos_sale_groupby.sql
-- con el guard agregado antes del bloque "Sesión de caja".
