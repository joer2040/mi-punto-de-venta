-- Hotfix 2: normalizar sales.payment_method a Title Case en finalize_pos_sale.
-- Causa raíz O3: v_dominant_method obtenido via trim(pay->>'method') sin CASE normalización.
-- Resultado: sales.payment_method = 'efectivo' (lowercase) excluida por EF loadSalesSummary
-- que filtra .eq('payment_method', 'Efectivo') (exact Title Case) → cash_sales=0 en get_session_overview.
-- Único cambio nuevo: select trim(...) → CASE normalización en v_dominant_method.
-- Hotfix 1 (financial_payments.payment_method CASE, 20260908100000) preservado idéntico.
-- Base: 20260908100000_fix_finalize_pos_sale_payment_method_case.sql (versión activa PRD).
...
begin
  ...
  -- Método dominante para campo legacy sales.payment_method — normalizado a Title Case
  -- Hotfix 2: trim(pay->>'method') → CASE para que loadSalesSummary (cash-operations)
  -- lo encuentre con .eq('payment_method', 'Efectivo').
  select
  ...
  -- ── Guard: bloquear venta si caja en proceso de cierre ───────────────────
  -- Aplica a todos los métodos de pago (efectivo, tarjeta, transferencia).
  if exists (
  ...
  if v_ledger_cutover_at is not null and v_sale_created_at >= v_ledger_cutover_at then
    ...
    -- Líneas de débito agrupadas por cuenta
    ...
    -- Pagos desglosados: normalizar a Title Case para satisfacer CHECK constraint
